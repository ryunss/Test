...
//		showNumber(1); // 무한정 재귀호출 안된다. : stackOverFlowError
...
//		int n = Integer.MAX_VALUE;
	...
	public static void showNumber(int n) {
		...
		showNumber(n+1); // 재귀호출 ?
		System.out.println("리턴: 찍힐까?"); // 출력하면 안나옴 계속 호출반복이라서 스택에 쌓이고 잇는중,,
			// 스택에 쌓이다가 넘어가면 스택오버플로우 되면서 에러 발생.
	}
	// 팩토리얼 고찰
	// 4! = 4 * 3 * 2 * 1 = 24
	// n! = n * (n-1) * (n-2) * ... * 1
	...
	public static long calcFactorial(long num) {
		...
		if(num == 0) { // 종료 조건
			...
			System.out.println("음수 팩토리얼은 계산할 수 없어");
		}
