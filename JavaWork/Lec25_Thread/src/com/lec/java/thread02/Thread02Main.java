...
public class Thread02Main {
	...
	public static void main(String[] args) {
		...
		// 3. Runnable을 구현하는 클래스의 인스턴스를 생성
		Runnable rb1 = new MyRunnable("안녕~ 자바!");
		Runnable rb2 = new MyRunnable("Hello~ Java!");
		Runnable rb3 = new MyRunnable("멀티쓰레드");
		...
		th1.start();
		th2.start();
		new Thread(rb3).start(); // 위랑 똑같음. 굳이 여러번 쓸 필요 없는 이름일 때
		new Thread(new MyRunnable("Java Thread")).start(); // 똑같음.
		...
		new Thread(new Runnable() {
			...
			public void run() {
				for(int i=1; i<10; i++) {
					System.out.println(Thread.currentThread().getName() + " " + i + " Anonymous class");
				}
			}
		}).start();
		...
		new Thread(()->{
			for(int i=1; i<10; i++) {
				System.out.println(Thread.currentThread().getName() + " " + i + " Lambda-expression");
				try {
					Thread.sleep(500);// 딜레이주는거, 이거 쓸려면 무조건 익셉션 잡아줘야함
