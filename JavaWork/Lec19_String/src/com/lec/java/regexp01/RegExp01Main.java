...
public class RegExp01Main {
	...
	public static void main(String[] args) {
		...
		regex = "My...."; // My 로 시작하고 임의의 문자 4개 // Mycode <= 매칭이 된다. My age <= " "스페이스도 문자라서 된다.
													 // Myage" <= 매칭이 안된다. mycode <= 매칭이 안된다.
...
//		  end() : 최근 매칭의 끝 index (마지막 매칭된 문자 '다음' 인덱스값)
		...
		if(matcher.find()) { // 어디까지 매칭했는지 기억하고 있음. 위에서 7번까지 봤으니 지금 if문은 7번부터 찾는거라 find() 실패로 나옴.
			System.out.println(matcher.group() + " {" + matcher.start() + "~"+matcher.end() + "} ");
		}else {
			System.out.println("find() 실패");
		}
		...
		matcher = pat.matcher("-My1234-"); // 매칭 실패
		...
		matcher = pat.matcher("My1234"); // 매칭 OK
		...
		if(Pattern.matches("My....", "Myabdc")) {
			System.out.println("Pattern.matches() 매칭OK");
		}else {
			System.out.println("Pattern.matches() 매칭FAIL");			
		}
