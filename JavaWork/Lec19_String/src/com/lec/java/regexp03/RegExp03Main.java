...
//		─────────────────────────────────────────
		...
		regex = "[abc]"; // a 또는 b 또는 c중에 한 문자에 매칭됨
		arrInput = new String[] {
				"able", // 1
				"bible",  // 2
				"cable",  // 3
		...
		regex = "[abc]+"; // abc중 한 문자가 꼭 한개이상 있어야된다.
		regex = "[a-z]+"; // a-z까지 한 문자가 꼭 한개이상 있어야된다.
		arrInput = new String[] {
				"abc100",
		...
		regex = "ab{2,}"; // a 다음에 오는 b의 개수가 2개 이상
		regex = "ab{3,5}"; // a 다음에 오는 b의 개수가 3~5개
		//─────────────────────────────────────────
		title = " () : ()안에 있는 글자들을 그룹화 ";
		regex = "a(bc)*"; // a 뒤에 (bc)가 없거나 하나 이상 있거나.
		arrInput = new String[] {
				"abc",
				"abcbcbbac",
				"abcabcabc",
