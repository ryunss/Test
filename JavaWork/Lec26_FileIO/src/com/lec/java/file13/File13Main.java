...
//			br = new BufferedReader(new InputStreamReader(new FileInputStream(FILE1))); 같은방식~
//			br = new BufferedReader(new InputStreamReader(new FileInputStream(new File(FILE1)))); 이거도~
			...
			System.out.println();
//			br = new BufferedReader(new InputStreamReader(new FileInputStream(FILE2))); // utf8
			// 저장된 인코딩과 다른 인코딩으로 읽으면 문자 깨진다.
			// 저장은 euc-kr로 했는데 읽는건 UTF로 되서 코딩깨지는거임.
			br = new BufferedReader(new InputStreamReader(new FileInputStream(FILE2), "euc-kr"));
			// 쓰는거 읽는거 둘다 euc-kr 이라 잘 읽어와짐
			test_read(br); 
