...
-- LIMIT from, page Rows (from 은 0부터 시작)
SELECT * FROM t4_write ORDER BY id DESC LIMIT 5;
SELECT * FROM t4_write ORDER BY id DESC LIMIT 5, 5; -- 0부터 시작함. 다섯번째부터 5개

-- 한 페이지에 10개씩 ?
SELECT * FROM t4_write ORDER BY id DESC LIMIT 0, 10; -- 1page
SELECT * FROM t4_write ORDER BY id DESC LIMIT 10, 10; -- 2page 10번째부터 10개
